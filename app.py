...
x_train, x_test, y_train, y_test = train_test_split(Data_tweet['tweet'].values, Data_tweet['Analysis'].values, test_size=0.30)
...
if st.button("SVM Classifier"):
  result=predict_sentiment([text])
  st.success('Model has predicted {}'.format(result))
if st.button("Navie Bayes Classification"):
  result=predict_sentiment([text])
  st.success('Model has predicted {}'.format(result)
